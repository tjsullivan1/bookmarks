...
class BookmarkBase(BaseModel):
    title: str = Field(
        ..., min_length=1, max_length=200, description="Title of the bookmark"
    )
    url: HttpUrl = Field(..., description="URL of the bookmark")
    description: Optional[str] = Field(
        None, max_length=500, description="Optional description"
    )
    category: BookmarkCategory = Field(
        default=BookmarkCategory.OTHER, description="Bookmark category"
    )
    tags: Optional[List[str]] = Field(
        default=[], description="List of tags for organization"
    )


class BookmarkCreate(BookmarkBase):
    user_id: str = Field(
        default="default_user", description="User ID for future multi-user support"
    )


class BookmarkUpdate(BaseModel):
    title: Optional[str] = Field(
        None, min_length=1, max_length=200, description="Title of the bookmark"
    )
